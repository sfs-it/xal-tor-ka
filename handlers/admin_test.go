...
func TestHostInternalize(t *testing.T) {
	cases := []struct {
		...
		{"docker non-local invariato", "host.docker.internal", "http://10.0.0.5:80", "http://10.0.0.5:80"},
		// Host deploy: localhost → 127.0.0.1
		{"host localhost", "127.0.0.1", "http://localhost:8765", "http://127.0.0.1:8765"},
		// Rewrite disabilitato: upstream invariato
		{"disabilitato", "", "http://localhost:8765", "http://localhost:8765"},
		// Senza schema: invariato
