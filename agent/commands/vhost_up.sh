#!/bin/bash
# Vetted: start one vhost of a site.
set -euo pipefail
...
docker compose --project-directory "$dir" -f "$f" -p "$(vhost_project "$name" "$vhost")" up -d
echo "started $name/$vhost -> $(vhost_alias "$name" "$vhost"):8080"
