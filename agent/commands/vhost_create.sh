...
tmpl="${XTK_P_TEMPLATE:-php-fpm}"; pv="${XTK_P_PHP_VERSION:-8.3}"; domain="${XTK_P_DOMAIN:-}"
...
[ -z "$domain" ] || [[ "$domain" =~ ^[a-z0-9]([a-z0-9.-]*[a-z0-9])?$ ]] || { echo "invalid domain" >&2; exit 2; }
...
al="$(render_vhost "$name" "$vhost" "$tmpl" "$pv" "$uid" "$gid" "$domain")" || exit $?
